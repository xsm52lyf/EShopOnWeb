terraform {
  ...
  backend "s3" {
    bucket         = "corecommerce-terraform-state-dev"
    key            = "corecommerce/dev/terraform.tfstate"
    region         = "us-east-1"
    encrypt        = true
    kms_key_id     = "alias/corecommerce-dev-state"
...
module "aws_network" {
  ...
  enable_nat_gateway = false  # 开发环境关闭 NAT 网关
  tags               = local.common_tags
}
