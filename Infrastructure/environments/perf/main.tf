terraform {
  ...
  backend "s3" {
    bucket         = "corecommerce-terraform-state-perf"
    key            = "corecommerce/perf/terraform.tfstate"
    region         = "us-east-1"
    encrypt        = true
    kms_key_id     = "alias/corecommerce-perf-state"
    dynamodb_table = "terraform-state-lock-perf"
  }
...
module "aws_network" {
  ...
  enable_nat_gateway = true  # 性能测试需要 NAT 网关
  tags               = local.common_tags
}
...
module "azure_dr_data" {
  ...
  sql_admin_password  = "TempPerfPassword123!"  # 实际应从 Secrets Manager 获取
...
module "aws_dms_dr" {
  ...
  environment                = local.environment
  replication_instance_id    = "corecommerce-perf-dr-replication"
  ...
  source_endpoint_id    = "corecommerce-perf-source"
  ...
  target_endpoint_id    = "corecommerce-perf-target-azure"
  ...
  replication_task_id = "corecommerce-perf-cdc-to-azure"
